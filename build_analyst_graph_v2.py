"""Build the corrected point-in-time common-analyst graph for the panel v3 event set.
...
v0.7. Changing any of them requires re-approval.
  ...
  8. The source and every candidate receiver must be an index member on the announcement day.
     Coverage values still come only from the latest strictly prior snapshot.
...
OUT_ROOT = ROOT / "data" / "analyst_graph_v2"
...
def load_coverage(plan: dict) -> tuple[pd.DataFrame, int, str]:
    frames = []
    manifest = {}
    for batch in plan["batches"]:
        csv_path = COVERAGE_RUN / f"{batch['batch_id']}.csv"
        meta_path = COVERAGE_RUN / f"{batch['batch_id']}.metadata.json"
        metadata = json.loads(meta_path.read_text(encoding="utf-8"))
        observed_hash = sha256_file(csv_path)
        if metadata.get("request_sha256") != batch["request_sha256"]:
            raise RuntimeError(f"request identity mismatch: {batch['batch_id']}")
        if metadata.get("csv_sha256") != observed_hash:
            raise RuntimeError(f"coverage file hash mismatch: {batch['batch_id']}")
        manifest[batch["batch_id"]] = observed_hash
    ...
    return data.reset_index(drop=True), before - len(data), manifest_hash
...
def main() -> int:
    ...
    coverage_data, duplicates_removed, coverage_manifest_hash = load_coverage(plan)
    ...
    source_member = events["source_in_index_on_announcement"]
    if source_member.dtype != bool:
        normalized = source_member.astype("string").str.lower()
        if normalized.isna().any() or (set(normalized.unique()) - {"true", "false"}):
            raise ValueError("invalid source_in_index_on_announcement values")
        source_member = normalized.eq("true")
    events["source_in_index_on_announcement"] = source_member.to_numpy()
    ...
    events.loc[~events.source_in_index_on_announcement, "graph_status"] = "source_not_member_on_announcement"
    events.loc[events.source_in_index_on_announcement & events.snapshot.isna(),
               "graph_status"] = "before_first_snapshot"
    events.loc[events.source_in_index_on_announcement & events.snapshot.notna() &
               events.snapshot_age_days.gt(MAX_SNAPSHOT_AGE_DAYS),
    ...
    for snapshot, group in events.loc[events.graph_status.eq("ok")].groupby("snapshot"):
        ...
        for row in group.itertuples(index=False):
            ...
            if source_position < 0:
                ...
                continue
            members = spans.loc[spans.member_from.le(row.announcement_day) &
                                spans.member_to.ge(row.announcement_day), "ric"]
    ...
    events[["source", "announcement", "period_end", "announcement_day",
            "source_in_index_on_announcement", "snapshot", "snapshot_age_days",
            "graph_status"]].to_csv(out / "event_graph_status.csv", index=False)
    ...
    summary = {
        ...
        "rules": {
            ...
            "candidate_membership_date": "announcement_day",
        },
        ...
        "inputs": {"plan_sha256": sha256_file(COVERAGE_RUN / "plan.json"),
                   "coverage_csv_manifest_sha256": coverage_manifest_hash,

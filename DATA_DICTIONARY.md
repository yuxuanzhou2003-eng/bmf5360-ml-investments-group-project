...
## AI-factor raw 与计划中的 clean/model-ready 表（2026-09-10）

以下 raw 表由 `ai_factor_collection_v1_config.json` 和 `collect_ai_factor_v1.py` 定义。raw 层保持供应商序列化，不做空值替换、去重、填补、缩尾、日期/时区/单位转换或标签计算。

| 表/请求族 | 原始粒度 | 主要字段 | 状态与限制 |
|---|---|---|---|
| `legacy_rd` | RIC × 供应商返回财务期 | R&D value、Date、Financial Period | 正式请求固定 `ReportingState=Orig`；Date 作为 period/date 字段保存，available time 另用原始公告表 gate |
| `legacy_revenue` | RIC × 财务期 | Revenue value、Date、Financial Period | 与 R&D 按明确期间键配对；不能按行位置拼接 |
| `f_tot_revenue/assets/com_eq/gross_profit/net_cfo/debt` | RIC × 财务期 | 六类原始财务控制 value/date/fperiod | 只用 Orig；用于 size/value/profitability/investment/leverage 控制的原料，不是已生成因子 |
| `income_statement_dates` | RIC × 财务期 | Original Announcement、Last Update、Period End | Original Announcement 是拟用可用时点；Last Update 只作修订诊断 |
| `business_segments_fy0` | RIC × 财务期 × raw segment row | segment code/name/fperiod/date/value | 保留 blank、total candidate、重复和 reconciliation gap；AI 词典尚未冻结 |
| `market_capitalization_monthly` | RIC × month | market cap、Date | value/date 同一请求；尚未做单位与币种转换 |
| `etf_daily_price_volume` | ETF RIC × 交易日 | price/OHLC/volume/bid/ask/turnover | SOXX、XLK、BOTZ、AIQ、IGV 独立请求；SPY 仅显式对账；价格收益不能自动称为总回报 |

正式 clean 计划输出 `fundamental_observations.csv`、`segment_observations.csv`、`etf_prices.csv`、`asset_returns.csv`、`ai_market_proxies.csv`、`membership_snapshots.csv`、`identity_map.csv`、`quarantine.csv` 和 `missingness_summary.csv`。正式 model-ready 计划输出以 `formation_session + security_id` 为主键的 `factor_scores.csv`、`factor_portfolio_members.csv`、`rf_features.csv`、独立封存的 `rf_targets.csv`、`eligibility.csv` 和 `purge_audit.csv`。这些表目前是契约，不是已经生成的数据。

"""Expanded independent validation of the broker coverage collection.
...
def main() -> int:
    ...
    instrument_exact, meta_instruments_ok = [], []
    frames = []
    for batch in batches:
        ...
        returned = set(frame["Instrument"].dropna().astype(str)) if "Instrument" in frame else set()
        requested_set = set(batch["request"]["universe"])
        instrument_exact.append(returned == requested_set)
        meta_instruments_ok.append(int(meta.get("instruments_returned", -1)) == len(returned))
    ...
    checks["each_batch_returns_exact_requested_instruments"] = bool(instrument_exact) and all(instrument_exact)
    checks["metadata_instrument_counts_match_files"] = bool(meta_instruments_ok) and all(meta_instruments_ok)
    checks["spans_hash_matches_collection_plan"] = plan.get("spans_sha256") == sha256_file(SPANS_PATH)
    checks["fields_match_approved_request"] = plan.get("fields") == [
    ...
    if date_column:
        ...
        checks["covered_activation_dates_parse"] = bool(
            pd.to_datetime(data.loc[data[broker_column].notna(), date_column], errors="coerce").notna().all())
        future_rows = int(future.sum())
    else:
        checks["no_recommendation_activated_after_snapshot"] = False
        checks["covered_activation_dates_parse"] = False
        future_rows = -1
    ...
    report = {
        ...
        "validator": "validate_broker_coverage_v2.py",
        ...
        "collector_sha256": sha256_file(ROOT / "collect_broker_coverage_v1.py"),

"""Leakage-corrected training/validation read on the common-analyst graph.
...
OUT_ROOT = ROOT / "data" / "analysis" / "analyst_signal_quickcheck_v2"
...
def main() -> int:
    ...
    # Seal the test window before any edge join or target lookup. Test-period edges never enter
    # attach_targets and therefore never receive a target even transiently in memory.
    event_level = events.loc[events.announcement_day.le(pd.Timestamp("2022-12-31")),
                             EVENT_KEY + ["announcement_day", "entry_session",
                                          "standardized_surprise"]].copy()

    returns = pd.read_csv(RETURNS, usecols=["Instrument", "Date", "return_decimal"])
    returns["Date"] = pd.to_datetime(returns.Date).dt.normalize()
    returns = returns.loc[returns.Date.lt(pd.Timestamp("2023-01-01"))].copy()
    ...
    def attach_targets(edges: pd.DataFrame, strength_columns: list[str]) -> pd.DataFrame:
        ...
        frame["boundary_purged"] = (~valid) | (frame.exit_session.notna() & frame.exit_session.ge(limit))
        ...
        safe = (receiver_index >= 0) & ~frame.boundary_purged.to_numpy()
        receiver_forward[safe] = matrix[entry_position[safe], receiver_index[safe]]
        benchmark_forward = np.full(len(frame), np.nan)
        benchmark_forward[safe] = spy_forward.to_numpy()[entry_position[safe]]
        frame["target"] = receiver_forward - benchmark_forward
            ...
            for split in ["training", "validation"]:
                ...
                for signal in signals:
                    ...
                    results.append({
                        ...
                        "rows": int(part.loc[part[signal].notna() &
                                              part.target.notna()].shape[0]),
                        "events": int(part.loc[part[signal].notna() & part.target.notna(),
                                               EVENT_KEY].drop_duplicates().shape[0]),
    ...
    summary = {
        ...
        "checks": {
            "development_events_end_before_test": bool(
                event_level.announcement_day.max() < pd.Timestamp("2023-01-01")),
            "new_frame_contains_only_training_validation": bool(
                set(new_frame.split.unique()) <= {"training", "validation"}),
            "old_frame_contains_only_training_validation": bool(
                set(old_frame.split.unique()) <= {"training", "validation"}),
            "return_matrix_ends_before_test": bool(returns.Date.max() < pd.Timestamp("2023-01-01")),
            "boundary_rows_have_no_target": bool(
                new_frame.loc[new_frame.boundary_purged, "target"].isna().all() and
                old_frame.loc[old_frame.boundary_purged, "target"].isna().all()),
        },
        "input_hashes": {
            "graph_edges": sha256(GRAPH / "edges.csv"),
            "panel_events": sha256(PANEL / "events.csv"),
            "returns": sha256(RETURNS),
            "trbc_diagnostic": sha256(TRBC),
            "code": sha256(Path(__file__).resolve()),
        },
        "output_hash": sha256(out / "quickcheck_metrics.csv"),
    }
    ...
    if not all(summary["checks"].values()):
        raise RuntimeError(summary["checks"])
    return 0
